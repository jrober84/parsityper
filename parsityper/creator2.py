...
from parsityper.constants import  bases_dict, SCHEME_HEADER
import cProfile
...
def qa_scheme(scheme,missing_targets, multiplicity_targets,min_len,max_len,max_ambig,min_complexity):
    ...
    for kmer_id in scheme:
        ...
        kmer_number = 0
        for seq in [pos_kmer,neg_kmer]:
            value = 1
            for char in seq:
                length_key = len(bases_dict[char])
                value *= length_key
            kmer_number += value
        scheme[kmer_id]['count_expanded_kmers'] = kmer_number
        num_bases_found = 0
        sum_gc = 0
        for b in bases:
            if b in base_counts:
    ...
    for kmer_id in scheme:
        ...
        for sample in scheme[kmer_id][column_name]:
            if sample in genotype_mapping:
                ...
                temp.append(sample)
        temp = list(set(temp))
        scheme[kmer_id][column_name] = ','.join([str(x) for x in temp])
    return scheme

def find_snp_kmers(input_alignment,ref_id,snp_positions,max_states,min_length,max_length):
    for pos in snp_positions:
        ...
        for seq_id in input_alignment:
            kmer = input_alignment[seq_id][start:end]
            if seq_id == ref_id:
                kmers['ref'] = kmer
            else:
                if kmer not in kmers['alts']:
                    kmers['alts'][kmer] = []
                kmers['alts'][kmer].append(seq_id)
        num_states = 1 + len(kmers['alts'])
...
def run():
    ...
    for kmer_id in scheme:
        ...
        positives.sort()
        partials.sort()
        scheme[kmer_id]['positive_seqs'] = ','.join([str(x) for x in positives])
